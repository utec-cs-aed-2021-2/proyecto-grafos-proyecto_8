...
template<typename TV, typename TE>
vector<TE> Best_BFS<TV, TE>::apply() {
    map<Vertex<TV , TE>* , bool > visitados;vector<Heuristica<TV , TE> *> preresultado;
    priority_queue<Heuristica<TV , TE> *  , vector< Heuristica<TV , TE> *> , Heuristica_menor<TV ,TE> > pQueue;
    Vertex<TV , TE> * inicio_vertex = graph->vertexes[inicio];Vertex<TV , TE> * final_vertex = graph->vertexes[final];
    // Inicializar la cola de prioridad con el nodo de inicio.
    pQueue.push(new Heuristica<TV , TE>(inicio_vertex));visitados[inicio_vertex] = true;
    // Realizar el algoritmo de busqueda de la ruta perfecta hasta que la cola de prioridad quede vacia
    while (!pQueue.empty()){
        Heuristica<TV , TE> * temp = pQueue.top();preresultado.push_back(temp);pQueue.pop();
        // Condicion para hallar el nodo final
        if(temp->vertice == final_vertex){
            Heuristica<TV , TE> * temp2 = temp;vector<TE> resultado;
            while(temp2 != nullptr){
                resultado.push_back(temp2->vertice->data);
                temp2 = verificacion(temp2->padres , preresultado);
            }
            reverse(resultado.begin() , resultado.end());return resultado;
        }
        ...
        for (Edge<TV , TE> * arista : temp->vertice->edges) {
            ...
            if(it == visitados.end()){
                visitados[temp_edge] = true;auto * temp_final = new Heuristica<TV ,TE>(temp_edge);
                temp_final->distancia = arista->weight;temp_final->padres = temp->vertice;
